...
from src.models.fs_track.interfaces import IJobCreator, ICSRFHandler
...
class JobCreator(IJobCreator):
    ...
    async def create_job(self, session: aiohttp.ClientSession, ticket: Ticket) -> int:
        ...
        async with session.post(f"{self.base_url}/admin/jobs/add", data=payload) as response:
    ...
    async def create_job_detail(self, session: aiohttp.ClientSession, ticket: Ticket) -> int:
        ...
        async with session.post(
            f"{self.base_url}/admin/job-details/edit/{last_id}", 
            data=payload
        ) as response:
    ...
    def _payload_jobs_detail(self, ticket: Ticket, csrf_token: str, jobs_id: int) -> FormData:
        form = FormData()
